...
<header @if($headerId) id="{{ $headerId }}" @endif class="relative">
    <div class="fixed inset-x-0 top-0 z-50">
        ...
        >
                ...
                <div class="flex items-center justify-between gap-4 lg:grid lg:grid-cols-[1fr_auto_1fr] lg:gap-6">
                    ...
                    <div class="flex items-center justify-end gap-3 lg:justify-self-end">
                        ...
                        <button
                            ...
                            class="flex h-11 w-11 shrink-0 aspect-square items-center justify-center rounded-full border {{ $isLanding ? 'border-white/25 bg-white/15 text-white shadow-none backdrop-blur' : 'border-slate-300 bg-white text-slate-800 shadow-sm backdrop-blur' }} transition hover:bg-white lg:hidden"
            ...
            <div id="mobile-menu" data-mobile-menu class="hidden border-t {{ $isLanding ? 'border-white/15 bg-[rgb(var(--agency-navy-1)/0.98)]' : 'border-slate-200/80 bg-white/95' }} backdrop-blur-xl lg:hidden">
                <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
                    <nav class="grid gap-2 text-sm font-semibold {{ $isLanding ? 'text-slate-100' : 'text-slate-800' }}" aria-label="Navigasi Menu Mobile">
                        @foreach ($navItems as $item)
                            @php
                                $isActive = $active === $item['key'];
                            @endphp
                            <a wire:navigate class="flex items-center justify-between rounded-xl px-3 py-2.5 transition-colors {{ $isActive ? 'bg-sky-500/15 text-sky-300 font-bold border-l-4 border-sky-400 pl-3' : ($isLanding ? 'hover:bg-white/10' : 'hover:bg-slate-100') }}" href="{{ $item['url'] }}">
